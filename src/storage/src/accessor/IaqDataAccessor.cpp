...
IDataJob::Ptr
IaqDataAccessor::makePutJob(const std::string& input, PutCallback callback)
{
    if (auto model = std::make_unique<IaqDataModel>(); model->parse(input)) {
        ...
        throw std::runtime_error{"Failed to parse input for IaqDataModel"};
    }
}

IDataJob::Ptr
IaqDataAccessor::makeGetJob(int64_t from, int64_t to, GetCallback callback)
{
