...
IDataJob::Ptr
TemperatureDataAccessor::makePutJob(const std::string& input, PutCallback callback)
{
    if (auto model = std::make_unique<TemperatureDataModel>(); model->parse(input)) {
        ...
        throw std::runtime_error{"Failed to parse input for TemperatureDataModel"};
    }
}

IDataJob::Ptr
TemperatureDataAccessor::makeGetJob(int64_t from, int64_t to, GetCallback callback)
{
